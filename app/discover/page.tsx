        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button 
              ...
              className={`flex items-center gap-2 bg-void-900 border border-zinc-700 px-5 py-2.5 rounded-full text-sm font-bold text-white transition-all shadow-md hover:bg-zinc-800 active:scale-95 ${genres.length > 0 || year !== 'all' ? 'border-cyan-400 text-cyan-400' : ''}`}
            >
              <SlidersHorizontal size={16} />
              Filters {(genres.length > 0 || year !== 'all') && <span className="bg-cyan-400 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full ml-1">!</span>}
            </button>
            <button 
      ...
      <div className="relative z-10 max-w-[1800px] mx-auto min-h-[50vh]">
        {loading ? (
          <div className="flex items-center justify-center h-64">
            <div className="w-10 h-10 rounded-full border-4 border-t-transparent animate-spin border-cyan-400/30 border-t-cyan-400" />
          </div>
        ) : results.length > 0 ? (
          <>
